...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 获取 easy-mock 的模拟数据
            getData() {
                this.$axios.get("/contract/contractsByPage",{
                    params:{
            ...
            handleDelete(index, row) {
                ...
                this.$confirm('确定要删除吗？', '提示', {
                    ...
                    .then(() => {
                        this.$axios.delete("/contract/contract/" + this.form.id).then(res => {
            ...
            saveEdit() {
                this.$refs.form.validate(validate => {
                    if (validate) {
                        ...
                        this.$axios.post("/contract/updateContract" , this.$qs.stringify(this.form)).then(res => {
                            if (res.data.result.resultCode == 200) {
            ...
            saveAdd(){
                ...
                this.$refs.form.validate(validate =>{
                    if(validate){
                        this.form.empId = this.empId;
                        this.$axios.post("/contract/contract",this.$qs.stringify(this.form)).then(res=>{
                            if(res.data.result.resultCode==200){
                                this.addVisible = false;
                                this.getData();
                            }
